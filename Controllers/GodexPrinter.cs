...
namespace EzioDll
{
    ...
    public class GodexPrinter
    {
        ...
        public static List<string> GetPrinter_USB()
        {
            byte[] byteUsbID = new byte[100];
            List<string> PrinterList = new List<string>();

            if (EZioApi.FindFirstUSB(byteUsbID) == 1)
            {
                PrinterList.Add(GetStringData(byteUsbID));
                while (EZioApi.FindNextUSB(byteUsbID) == 1)
                {
                    PrinterList.Add(GetStringData(byteUsbID));
                }
            }
        ...
        private static string GetStringData(byte[] byteData)
        {
            string strData = Encoding.ASCII.GetString(byteData);
            int Idx = strData.IndexOf("\0");
            if (Idx > 0)
                strData = strData.Substring(0, Idx);
            return strData;
        }
        ...
        public void OpenUSB(string PortName)
        {
            EZioApi.OpenUSB(PortName);
        }
        ...
        public int Download_FW(string FileName)
        {
            return EZioApi.Download_FW(FileName);
        }
    }
